...
- 向指定环境的全部有效普通 APNs token 发送全量通知/广告，支持活动 ID、App 跳转链接、自定义数据和仅预估受众的 dry-run。
...
全量通知接口为 `POST /push/admin/api/send/broadcast`。它会固定本次受众的 token ID 上界并分页投递，避免一次加载全部设备；这是普通 APNs alert 的服务端 fan-out。Apple 的 Broadcast Push Notifications 仅支持 iOS 18 及以上的 Live Activities channel，不能用于普通广告横幅。

PushKit 调试请求只验证 APNs 与 PushKit 通道，不会创建 CallService session，因此不能替代完整通话测试。普通消息调试默认遵循私聊 `is_notify` 设置，页面可显式选择仅本次忽略。

紧急通知的调研和后续实施约束见 [URGENT_NOTIFICATIONS.md](./URGENT_NOTIFICATIONS.md)。
